...
function fetchSleepData(accessToken)  {
  ...
  fetch(`https://api.fitbit.com/1.2/user/6XJVRS/sleep/date/${todayDate}.json`, {
    method: "GET",
    headers: {
...
function fetchHeartRate(accessToken)  {
  let date = new Date();
  let todayDate = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`; //YYYY-MM-DD

  // heart rate api
  fetch(`https://api.fitbit.com/1/user/[user-id]/activities/heart/date/${todayDate}/1d.json`, {
    method: "GET",
    headers: {
      "Authorization": `Bearer ${accessToken}`
    }
  ...
  .then(function(data) {
    let myData = {
      restingHeartRate: data.value.restingHeartRate
    }
    if (messaging.peerSocket.readyState === messaging.peerSocket.OPEN) {
      messaging.peerSocket.send(myData);
    }
...
settingsStorage.onchange = evt => {
  if (evt.key === "oauth") {
    ...
    fetchHeartRate(data.access_token) ;
  }
};
